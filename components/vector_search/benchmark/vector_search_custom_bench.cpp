// End-to-end SQL-level benchmark for vector search.
//
// Measures, per (N rows, D dim, K, filter selectivity, strategy):
//   - average latency
//   - p95 latency
//   - throughput (QPS, measured over a fixed time window)
//   - peak RSS delta (RAM) via getrusage
//   - user/sys CPU time delta via getrusage
//
// Backs the diploma's "Экспериментальное исследование и оценка производительности"
// section: avg/p95 latency, throughput, scalability over N/D/K, and resource utilization.
...
namespace {
    ...
    // Run `query` a fixed number of times, gathering per-call wall-clock latency
    // and an aggregate QPS over the same window. Resource deltas come from getrusage.
...
TEST_CASE("vector_search_bench::scalability_over_N") {
    ...
    for (std::size_t N : row_counts) {
        ...
        std::string vs_q = "SELECT * FROM " + db_name + "." + coll_name +
                           " ORDER BY vec_distance(embedding, " + q_str + ", 'l2') LIMIT " +
                           std::to_string(k) + ";";
...
TEST_CASE("vector_search_bench::filter_strategy_comparison") {
    ...
    for (const auto& sel : selectivities) {
        ...
        std::string pre_q = "SELECT * FROM " + db_name + "." + coll_name + sel.predicate +
                            " ORDER BY vec_distance(embedding, " + q_str + ", 'l2', 'pre') LIMIT " +
                            std::to_string(k) + ";";
        std::string post_q = "SELECT * FROM " + db_name + "." + coll_name + sel.predicate +
                             " ORDER BY vec_distance(embedding, " + q_str + ", 'l2', 'post') LIMIT " +
                             std::to_string(k) + ";";

        benchmark_query(dispatcher, session, "pre_filter", pre_q, iterations);
        benchmark_query(dispatcher, session, "post_filter", post_q, iterations);
    }
}
